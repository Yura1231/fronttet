...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!userId) {
        console.error("Помилка: userId не знайдено в URL!");
        return;
    }

 
    
    fetch(`https://newsse-852faa67b80a.herokuapp.com/profile/${userId}/`, {
        method: "GET",
        headers: {
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    async function fetchReviews() {
        try {
            let response = await fetch(`https://newsse-852faa67b80a.herokuapp.com/comments/${userId}/`, {
                method: "GET",
                headers: {
                    "Authorization": `Bearer ${accessToken}`,
                    "Content-Type": "application/json"
